...
/**
 * Banner height — taller on small screens so offer copy can wrap;
 * single-line `h-9` from `sm` up. Keep nav / main offsets in sync.
 */
export const LAUNCH_BANNER_HEIGHT = "h-[3.25rem] sm:h-9"
...
/** Nav sits under the fixed banner on small screens; sticky from `lg`. */
export const NAVBAR_HEADER_CLASS =
  "fixed inset-x-0 top-[3.25rem] z-50 bg-background/45 backdrop-blur-xl backdrop-saturate-150 sm:top-9 lg:sticky lg:inset-x-auto lg:top-0"

/** `<main>` offset under fixed banner + nav (3.5rem) below `lg`. */
export const MARKETING_MAIN_OFFSET_CLASS =
  "pt-[calc(3.25rem+3.5rem)] sm:pt-[calc(2.25rem+3.5rem)] lg:pt-0"
