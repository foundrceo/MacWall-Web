...
export default function AdminAnalyticsPage() {
  ...
  const avgLikes =
    data && data.catalogWallpaperCount > 0
      ...
      : 0
  const activatedAllTime = data?.activatedDevicesAllTime ?? 0
  ...
  const rangeLabel =
    data?.rangeDays === 0 ? "all-time" : `last ${data?.rangeDays ?? days} days`
              ...
              {live ? (
                ...
                >
                  <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <StatCard
                      icon={
                        <Radio className="size-4 animate-pulse text-emerald-500 motion-reduce:animate-none" />
                      }
                      label="Active now (5m)"
                      ...
                      {live.currentActions.length === 0 ? (
                        ...
                        <div className="space-y-3 py-2">
                          {live.currentActions.map((act) => {
                            const totalActs = live.currentActions.reduce(
                              (acc, a) => acc + a.count,
                              0
                            )
                            const pct =
                              totalActs > 0
                                ? Math.round((act.count / totalActs) * 100)
                                : 0
                            return (
                            ...
                            >
                              ...
                              <div className="flex shrink-0 items-center gap-2">
                                ...
                                <span className="text-[11px] text-[var(--admin-muted)]">
                                  {feed.agoSeconds < 60
                                    ? `${feed.agoSeconds}s ago`
                                    : `${Math.round(feed.agoSeconds / 60)}m ago`}
                                </span>
                              </div>
                            </li>
                          ))}
                ...
                >
                  ...
                  <div className="grid gap-4 lg:grid-cols-3">
                    <Panel
                      ...
                      description="Paid licenses only. Pro is 3 Macs. Pro+ is 5 or more."
                    >
                      <LicensePlanPieChart rows={licenses.planBreakdown} />
                    </Panel>
                    <Panel
                      title="License Status Breakdown"
                      description="Every row in macwall_licenses"
                    >
                      <LicenseStatusDonut rows={licenses.statusBreakdown} />
                    </Panel>
                    <Panel
                      title="Top Buyer Geo-Locations"
                      description="visitor_country on paid licenses"
                    >
                      <RankedBarList
                      ...
                      hint={
                        data.opsLive?.cloudflare.available
                          ? data.opsLive.cloudflare.buckets
                              .map(
                                (bucket) =>
                                  `${bucket.name}${bucket.reachable ? "" : " down"}`
                              )
                              .join(" · ")
                          : (data.opsLive?.cloudflare.error ??
                  ...
                  <div className="grid gap-4 lg:grid-cols-2">
                    ...
                    >
                      {(data.stripeLive?.promotions ?? []).length > 0 ? (
                        <RankedBarList
                          rows={(data.stripeLive?.promotions ?? []).map(
                            (promo) => ({
                              label: promo.code,
              ...
              >
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  <Panel title="Download Clicks by Button Location">
                    <RankedBarList
                      rows={(data.downloadClicksByLocation ?? []).map(
                        (row) => ({
                          label: row.location,
                          value: row.count,
                        })
                    ...
                    >
                      <RingGauge
                        value={
                          feedback.total > 0
                            ? Math.round(
                                (feedback.resolved / feedback.total) * 100
                              )
                            : 100
                        }
