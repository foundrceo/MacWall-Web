...
/** ~pathLength spring visual duration */
const TICK_DRAW_MS = 400
const BADGE_FADE_MS = 220
const LOGO_SWAP_MS = TICK_START_MS + TICK_DRAW_MS + BADGE_FADE_MS + 40

type Phase = "check" | "logo"
...
export function ThankYouSuccessMark({
  className,
}: Readonly<{
  className?: string
}>) {
  ...
  useEffect(() => {
    ...
    return () => globalThis.clearTimeout(toLogo)
  }, [reducedMotion, showLogo])

  return (
    <div
      className={cn("relative mx-auto mb-6", className)}
    ...
    >
      <AnimatePresence mode="wait" initial={false}>
        {phase === "check" ? (
          ...
          >
            <ThankYouBadgeIcon reducedMotion={reducedMotion} />
          </motion.div>
        ) : (
          <motion.div
            key="thank-you-logo"
