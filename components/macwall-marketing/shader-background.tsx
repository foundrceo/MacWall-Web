...
import type { ShaderUniformConfig } from "@/lib/affiliate-shader-presets"
import { AFFILIATE_SHADER_STEP_01 } from "@/lib/affiliate-shader-presets"
...
export function ShaderBackground({
  className,
  config = AFFILIATE_SHADER_STEP_01,
}: Readonly<{
  className?: string
  ...
  useEffect(() => {
    ...
    const loc = webgl.getAttribLocation(program, "a_position")
    webgl.enableVertexAttribArray(loc)
    webgl.vertexAttribPointer(loc, 2, webgl.FLOAT, false, 0, 0)

    const uni = {
      colors: webgl.getUniformLocation(program, "u_colors"),
      scene: webgl.getUniformLocation(program, "u_scene"),
      shape: webgl.getUniformLocation(program, "u_shape"),
      surface: webgl.getUniformLocation(program, "u_surface"),
      finish: webgl.getUniformLocation(program, "u_finish"),
      transform: webgl.getUniformLocation(program, "u_transform"),
      space: webgl.getUniformLocation(program, "u_space"),
      cursor: webgl.getUniformLocation(program, "u_cursor"),
    }

    applyStaticUniforms(webgl, uni, config)
    ...
    function render(now: number) {
      raf = 0
      if (disposed || !visible || !inView || !canvas) return
      ...
      webgl.uniform4f(uni.space, config.offsetX, config.offsetY, 0, 0)
      webgl.uniform4f(
        uni.cursor,
        0,
        config.cursorEffect,
