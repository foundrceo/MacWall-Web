...
import { adminPageTransition, adminSpring } from "@/components/admin/admin-motion"
...
function SidebarNav({
  ...
  return (
    <nav className="flex flex-col gap-0.5 px-3" aria-label="Admin primary">
      <p className="px-2.5 pt-1 pb-2 text-[11px] font-semibold tracking-[0.08em] text-[var(--admin-muted)] uppercase">
        Workspace
      </p>
      <LayoutGroup id="admin-sidebar-nav">
        {ADMIN_NAV.map(({ href, label, icon: Icon }) => {
          ...
          const badge =
            href === "/admin/feedback" && replyCount > 0 ? replyCount : 0
          ...
          return (
            <Link
              ...
              className={cn(
                "group relative flex h-9 items-center gap-2.5 rounded-xl px-3 text-[13px] font-medium outline-none transition-colors",
                active
                  ? "text-[var(--admin-fg)]"
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[var(--admin-sidebar-width)] flex-col border-r border-[var(--admin-border)] bg-[var(--admin-surface)]/95 backdrop-blur-md md:flex">
        ...
        <div className="shrink-0 space-y-1 border-t border-[var(--admin-border)] p-3">
          <div className="flex items-center gap-2 rounded-xl border border-[var(--admin-border)] bg-[var(--admin-canvas)] px-3 py-2">
            <AdminStatusDot tone="green" pulse label="Admin services reachable" />
